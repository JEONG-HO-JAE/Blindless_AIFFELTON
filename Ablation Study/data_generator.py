...
class AdaptiveDataGenerator(tf.keras.utils.Sequence):
    def __init__(self, dir_path, number_of_images = 4,
                 ...
                 is_train=True, is_test=False, slice_black=False, augmentation=None):
...
class FullSizedDataGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        for i, data in enumerate(batch_data):
            ...
            image = cv2.imread(os.path.join(self.dir_path, "Images", image_path))
            image = preprocess.apply_cutomized_preprocess(image)
            label = cv2.imread(os.path.join(self.dir_path, "Labels", label_path))
            label = cv2.cvtColor(label, cv2.COLOR_BGR2GRAY)
            label = ((label == 255).astype(np.uint8) * 1).reshape(label.shape + (1,))
...
class SlicedDataGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        for i in range(0, self.number_of_images):
            ...
            # print("i:", i)
            image_path, label_path = batch_data[i]
            image = cv2.imread(os.path.join(self.dir_path, "Images", image_path))
            image = preprocess.apply_cutomized_preprocess(image)
            label = cv2.imread(os.path.join(self.dir_path, "Labels", label_path))
            label = cv2.cvtColor(label, cv2.COLOR_BGR2GRAY)
...
class SlicedDataGeneratorForTest(tf.keras.utils.Sequence):
    def __init__(self, dir_path, file_name, number_of_images = 2,
                 img_size=(512, 512, 1), output_size=(512, 512, 1), resize_shape = (1024, 1024, 1),
                 is_train=True, is_test=False, slice_black=False, augmentation=None):
        self.dir_path = dir_path
...
class FullSizedDataGeneratorForTest(tf.keras.utils.Sequence):
    def __init__(self, dir_path, file_name, batch_size=4,
                 img_size=(512, 512, 1), output_size=(512, 512, 1),
                 is_train=True, is_test=False, slice_black=False, augmentation=None):
        self.dir_path = dir_path
        self.file_name = file_name
...
class SlicelSizedDataGeneratorForTest(tf.keras.utils.Sequence):
    def __init__(self, dir_path, file_name, number_of_images = 2,
                 img_size=(512, 512, 1), output_size=(512, 512, 1), resize_shape = (1024, 1024, 1),
                 is_train=True, is_test=False, slice_black=False, augmentation=None):
        self.dir_path = dir_path
...
class AdaptiveSizedDataGeneratorForTest(tf.keras.utils.Sequence):
    def __init__(self, dir_path, file_name, number_of_images = 4,
                 img_size=(512, 512, 1), output_size=(512, 512, 1),
                 is_train=True, is_test=False, slice_black=False, augmentation=None):
        self.number_of_images = number_of_images
        self.dir_path = dir_path
    ...
    def  __getitem__(self, index):
        ...
        for image_path, label_path in batch_data:
            ...
            label = cv2.imread(os.path.join(self.dir_path, "Labels", label_path))
            label = cv2.cvtColor(label, cv2.COLOR_BGR2GRAY)
            label = ((label == 255).astype(np.uint8) * 1)
        ...
        augmented_data = {"image": image_patches,
                          "mask": label_patches}     
        augmented_data = self.augmentation(**augmented_data)
