...
class FR_UNet(tf.keras.Model):
    ...
    def call(self, x):
        ...
        if self.out_ave:
            ...
            output = self.fuse_layer(tf.concat([self.final1(x1_1), self.final2(x10),
                                                self.final3(x11), self.final4(x12), self.final5(x13)], axis=-1))
        return output

# model = FR_UNet(num_classes=1, feature_scale=2, dropout=0.2, fuse=True, out_ave=True)
